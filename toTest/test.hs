...
instance Foldable Tree where
  ...
  -- countNodes = treeFoldr (+) 0 $ treeMap (const 1)
...
moveList :: [Int] -> Int -> Maybe Int
moveList [] x = Just x
moveList (xs:x) n = move xs n >>= move_list x

removeDups = head.group.sort
removeDups2 = toList.fromList
-- Zad 9
moveKnight :: (Int, Int) -> (Int, Int) -> [(Int, Int)]
moveKnight (n,k) (x,y) = map removeDups2 [(x+i, y+j) | i <- [1,-1], j <- [2,-2], abs(x+i) < n, (x+i)> 0, (y+j) > 0, abs (y+j) < k]
...
removeDups ls1 ls2 = [x | x <- ls1, not (isMember x ls2)]