...
b :: (Double,Double) -> Integer -> (Double,Double)
b (sm,lst) x = let el = lst / fromInteger x
               in (sm + el, el)
approxE2 :: Integer -> Double
approxE2 n = fst (foldl b (1.0,1.0) [1..n])

-- MAIN -----------------------------------------------------------------------
someFunc :: IO ()
someFunc = do
  print "This is a library for excercises from lab2"
  -- ex 1 -------------------------------------------------------------------------
  --putStrLn "ex1 : Sum of squares --------------------------------------------------------------------------------------------------------"
  timeIt $ print (squareSum [1..3])
  -- ex 2 -------------------------------------------------------------------------
  --putStrLn "ex2 : Sum of squares of primes  --------------------------------------------------------------------------------------------------------"
  timeIt $ print (squareSumPrime [1..5])
  -- ex 3 -------------------------------------------------------------------------
  putStrLn "ex3 : How many even --------------------------------------------------------------------------------------------------------"
  timeIt $ print (countEven a)
  -- ex 4 -------------------------------------------------------------------------
  putStrLn "ex4 : mean --------------------------------------------------------------------------------------------------------"
  print(countAv [1,2])
  -- ex 5 -------------------------------------------------------------------------
  putStrLn "ex5 : Map with foldr and foldl --------------------------------------------------------------------------------------------------------"
  print(mymap (^2) arrr)
  print(testmap [1..5])
  --print(foldl (+) 0 (map (>0) [1,-1,-1,2]))
  -- ex 6 -------------------------------------------------------------------------
  --putStrLn "ex6 : Euler --------------------------------------------------------------------------------------------------------"
  ...
  timeIt $ print(map approxE2 [1..10])
  where n = 31
        a = [1..23]
        a3 = 18
        n2 = 35 :: Int
